 ...
 *
 * TLSF achieves O(1) cost for malloc and free operations by limiting the search
 * for a free block to a free list of guaranteed size adequate to fulfill the
 * request, combined with efficient free list queries using bitmasks and
 * architecture-specific bit-manipulation routines.
 *
 * Most modern processors provide instructions to count leading zeroes in a
 * word, find the lowest and highest set bit, etc. These specific
...
/* gcc 3.4 and above have builtin support, specialized for architecture. Some
 * compilers masquerade as gcc; patchlevel test filters them out. */
...
tlsf_decl int tlsf_ffs(uint32_t word)
{
...
tlsf_decl int tlsf_fls(uint32_t word)
{
...
tlsf_decl int tlsf_fls_generic(uint32_t word)
{
    int bit = 32;
...
enum tlsf_public {
    /* log2 of number of linear subdivisions of block sizes. Larger values
     * require more memory in the control structure. Values of 4 or 5 are
     * typical.*/
...
enum tlsf_private {
     ...
     * or (1 << (SL_INDEX_COUNT_LOG2 + 2)) bytes, as there we will be trying
     * to split size ranges into more slots than we have available. Instead,
     * we calculate the minimum threshold size, and place all blocks below
     * that size into the 0th first-level list. */

#if defined (TLSF_64BIT)
    /* TODO: We can increase this to support larger sizes, at the expense of
     *       more overhead in the TLSF structure. */
    FL_INDEX_MAX = 32,
#else
    FL_INDEX_MAX = 30,
#endif
 ...
 * prev_phys_block field, and no larger than the number of addressable bits for
 * FL_INDEX. */
...
{
    return tlsf_cast(void *, tlsf_cast(uint8_t *, block) + block_start_offset);
}
...
/* Adjust an allocation size to be aligned to word size, and no smaller than
 * internal minimum. */
static size_t
...
search_suitable_block(control_t *control, int *fli, int *sli)
{
    ...
    if (!sl_map) {
        ...
        const uint32_t fl_map = control->fl_bitmap & (~0U << (fl + 1));
...
block_locate_free(control_t *control, size_t size)
{
    ...
    if (size) {
         ...
         * large sizes it can sometimes wind up with indices that are
         * off the end of the block array. So, we protect against that
         * here, since this is the only callsite of mapping_search. Note
         * that we don't need to check sl, since it comes from a modulo
...
static void
_default_walker(void *ptr [[maybe_unused]], size_t size [[maybe_unused]], int used [[maybe_unused]], void *user [[maybe_unused]])
{
}
...
int
tlsf_pool_check(tlsf_pool_t pool)
{
...
/*
 * Size of the TLSF structures in a given memory block passed to tlsf_create,
 * equal to the size of a control_t
 */
size_t
tlsf_size(void)
{
 ...
 * tlsf_add_pool, equal to the overhead of a free block and the sentinel
 * block. */
size_t
...
tlsf_pool_t
tlsf_pool_add(tlsf_t tlsf, void *mem, size_t bytes)
{
    block_header_t *block;
    block_header_t *next;
    ...
    if (pool_bytes < block_size_min || pool_bytes > block_size_max) {
        ...
        printf("tlsf_add_pool: Memory size must be between 0x%x and 0x%x00 bytes.\n",
          (uint32_t)(pool_overhead + block_size_min),
          (uint32_t)((pool_overhead + block_size_max) / 256));
#else
        /* printf("tlsf_add_pool: Memory size must be between %u and %u bytes.\n", */
        /*      (uint32_t)(pool_overhead + block_size_min), */
        /*      (uint32_t)(pool_overhead + block_size_max)); */
#endif
        return 0;
    }

    /* Create the main free block. Offset the start of the block slightly so
     * that the prev_phys_block field falls outside of the pool - it will
     * never be used. */
...
void
tlsf_pool_remove(tlsf_t tlsf, tlsf_pool_t pool)
{
    control_t *control = tlsf_cast(control_t *, tlsf);
...
tlsf_t
tlsf_pool_create(void *mem, size_t bytes)
{
...
tlsf_memalign(tlsf_t tlsf, size_t align, size_t size)
{
    ...
    const size_t size_with_gap = adjust_request_size(adjust + align +
            gap_minimum,
        align);

    /* If alignment is less than or equals base alignment, we're done. If we
     * requested 0 bytes, return null, as tlsf_malloc(0) does. */
    const size_t aligned_size = (adjust && align > ALIGN_SIZE) ? size_with_gap :
                                                                 adjust;
