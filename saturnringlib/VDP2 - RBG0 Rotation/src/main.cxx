...
{
    SRL::VDP2::RBG0::ScrollDisable();//turn off scroll display so we don't see junk when loading to VRAM
...
int main()
{
    ...
    while(1)
    {
        ...
        if (port0.IsConnected())
        {
            ...
            if (port0.WasPressed(Gamepad::Button::R))
            {
                ++CurrentMode;
            ...
            else if (port0.WasPressed(Gamepad::Button::L))
            {
                --CurrentMode;
                if (CurrentMode > 2)CurrentMode = 0;
                if (CurrentMode < 0)CurrentMode = 2;
            ...
            if (port0.IsHeld(Gamepad::Button::X)) angX += DEGtoANG(0.3);
            else if (port0.IsHeld(Gamepad::Button::A)) angX -= DEGtoANG(0.3);

            if (port0.IsHeld(Gamepad::Button::Y)) angY += DEGtoANG(0.3);
            else if (port0.IsHeld(Gamepad::Button::B)) angY -= DEGtoANG(0.3);

            if (port0.IsHeld(Gamepad::Button::Z)) angZ += DEGtoANG(0.3);
            else if (port0.IsHeld(Gamepad::Button::C)) angZ -= DEGtoANG(0.3);
        }
